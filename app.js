...
async function loadSpotlight(){
  ...
  try{
    ...
    try{
      const n=await fetch('https://60s.viki.moe/v2/60s');
      const nj=await n.json();
      if(nj&&nj.data){
        h+='<div class="daily-card"><div class="dc-title">» 60秒读懂世界 <span style="margin-left:auto;color:#999;font-size:10px">'+nj.data.date+' '+nj.data.day_of_week+'</span></div>';
        if(nj.data.news)nj.data.news.slice(0,12).forEach((x,i)=>{h+='<div class="dc-item" onclick="quickPost(\''+esc(x.slice(0,20))+'…\')"><b>'+(i+1)+'.</b> '+esc(x)+'</div>';});
        if(nj.data.tip)h+='<div class="dc-quote">"'+esc(nj.data.tip)+'"</div>';
        h+='<div class="dc-item" style="color:#7B68EE;font-weight:600" onclick="location.href=\''+(nj.data.link||'https://mp.weixin.qq.com')+'\'">📅 历史上的今天 & 完整日报 →</div>';
        h+='</div>';
        if(myProfile&&!localStorage.getItem('hxs_sp_'+nj.data.date)){
          localStorage.setItem('hxs_sp_'+nj.data.date,'1');
          try{await dbRpc('notify_user',{p_user_id:myProfile.id,p_type:'coin',p_content:'阅读今日简报+1经验+1硬币'});await dbUpdate('profiles','id=eq.'+myProfile.id,{exp:myProfile.exp+1,coins:myProfile.coins+1});myProfile.exp+=1;myProfile.coins+=1;}catch(e){}
        }
      }
    }catch(e){h+='<div class="daily-card"><div class="empty">简报加载中</div></div>';}
...
box.innerHTML=`<div class="profile-top"><div class="pt-avatar">${u.avatar_url?`<img src="${esc(u.avatar_url)}">`:esc((u.nickname||'友')[0])}</div><div class="pt-name">${esc(u.nickname)}${u.role==='admin'?'<span class="pt-tag">管理员</span>':''}</div><div class="pt-exp"><i style="width:${Math.max(4,pct)}%"></i></div><div class="pt-exp-t">Lv${u.level}·经验${u.exp}/${expNeedLv(u.level+1)}</div><div class="profile-bio">${esc(u.bio||'这个人很懒，什么都没写')}</div><div class="stat-card" style="margin-top:8px"><div><div class="stat-num" style="color:#ff9500">🪙${u.coins}</div><div class="stat-label">硬币</div></div><div><div class="stat-num" style="color:#7B68EE">${folN?folN.length:0}</div><div class="stat-label">关注</div></div><div><div class="stat-num" style="color:#6A5ACD">${fanN?fanN.length:0}</div><div class="stat-label">粉丝</div></div></div>${isMe?'':`<span class="follow-btn ${fol&&fol.length?'':'on'}" style="background:${fol&&fol.length?'#eee':'linear-gradient(135deg,#7B68EE,#6A5ACD)'};color:${fol&&fol.length?'#555':'#fff'}" onclick="${fol&&fol.length?'unfollowUser(\''+uid+'\')':'followUser(\''+uid+'\')'}">${fol&&fol.length?'已关注':'＋关注'}</span>`}</div>`;loadUserPosts('posts');goPage('userPage');}catch(e){toast('加载失败');}}
...
$('meProfile').innerHTML=`<div class="profile-top"><div class="pt-avatar" onclick="goPage('editPage')">${u.avatar_url?`<img src="${esc(u.avatar_url)}">`:esc((u.nickname||'友')[0])}</div><div class="pt-name">${esc(u.nickname)}${u.role==='admin'?'<span class="pt-tag">管理员</span>':''}<span class="pt-tag" style="background:#6A5ACD;margin-left:4px">💯${u.credit}分</span></div><div class="pt-exp"><i style="width:${Math.max(4,pct)}%"></i></div><div class="pt-exp-t">Lv${u.level}·经验${u.exp}/${expNeedLv(u.level+1)}·${checkinLevelTxt(u)}</div><div class="profile-bio">${esc(u.bio||'点击头像编辑资料')}</div><div class="stat-card" style="margin-top:8px"><div><div class="stat-num" style="color:#ff9500">🪙${u.coins}</div><div class="stat-label">硬币</div></div><div><div class="stat-num" style="color:#7B68EE">${folN.length}</div><div class="stat-label">关注</div></div><div><div class="stat-num" style="color:#6A5ACD">${fanN.length}</div><div class="stat-label">粉丝</div></div></div></div>`;
...
async function saveProfile(){if(!myProfile){toast('请先登录');needLogin();return;}const bio=$('eBio').value.trim();try{let avatar_url=myProfile.avatar_url||'';const f=$('eAvatar')._selected;if(f){avatar_url=await uploadImage(f);}await dbUpdate('profiles','id=eq.'+myProfile.id,{bio,avatar_url});myProfile.bio=bio;myProfile.avatar_url=avatar_url;toast('✅已保存');loadMe();goPage('mePage');}catch(e){$('editErr').textContent='保存失败：'+e.message;}}
...
async function delSelectedPosts(){const sel=[...document.querySelectorAll('.ap:checked')].map(x=>x.value);if(!sel.length){toast('请先选择帖子');return;}if(!confirm('确定删除选中的'+sel.length+'个帖子？'))return;for(const id of sel){try{await dbDelete('posts','id=eq.'+id);}catch(e){}}toast('✅已删除');loadAdmin('posts');}
