...
async function loadSpotlight(){
  ...
  try{
    ...
    try{
      ...
      if(nj&&nj.data){
        h='<div style="font-size:14px;font-weight:700;margin-bottom:6px">🗞️ 每日速递 '+nj.data.date+' '+nj.data.day_of_week+' '+nj.data.lunar_date+'</div>';
        if(nj.data.news)nj.data.news.forEach((x,i)=>{h+='<div class="sp-item">'+(i+1)+'. '+esc(x)+'</div>';});
        if(nj.data.tip)h+='<div style="font-style:italic;color:#888;margin-top:6px">📝 '+esc(nj.data.tip)+'</div>';
    ...
    }catch(e){h='<div class="empty">简报加载中</div>';}
    h+='<div style="font-size:13px;font-weight:700;margin-top:8px;margin-bottom:4px">🔥 实时热搜</div><div style="display:flex;flex-wrap:wrap;gap:6px">';
    [{n:'百度热搜',u:'https://top.baidu.com/board',c:'#2D7FF9'},{n:'微博热搜',u:'https://s.weibo.com/top/summary',c:'#e5484d'},{n:'抖音热榜',u:'https://www.douyin.com/hot',c:'#161823'},{n:'B站热搜',u:'https://www.bilibili.com/v/popular/rank/all',c:'#fb7299'},{n:'知乎热榜',u:'https://www.zhihu.com/hot',c:'#0084ff'},{n:'腾讯新闻',u:'https://news.qq.com',c:'#07c160'}].forEach(x=>{h+='<div style="background:'+x.c+';color:#fff;font-size:11px;padding:4px 10px;border-radius:12px;cursor:pointer" onclick="location.href=\''+x.u+'\'">'+x.n+'</div>';});
    h+='</div>';
    $('spotlight').innerHTML=h;
  }catch(e){$('spotlight').innerHTML='<div class="empty">日报加载失败</div>';}
...
async function likeComment(cid){try{const cur=await api('/rest/v1/comments?select=like_count&id=eq.'+cid);await dbUpdate('comments','id=eq.'+cid,{like_count:(cur&&cur[0]?cur[0].like_count:0)+1});toast('👍已点赞');loadComments(currentPost.id);}catch(e){toast('点赞失败');}}
