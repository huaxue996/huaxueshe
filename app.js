...
const HOT_LINKS=[{n:'百度热搜',u:'https://top.baidu.com/board'},{n:'微博热搜',u:'https://s.weibo.com/top/summary'},{n:'抖音热榜',u:'https://www.douyin.com/hot'},{n:'B站热搜',u:'https://www.bilibili.com/v/popular/rank/all'},{n:'知乎热榜',u:'https://www.zhihu.com/hot'},{n:'腾讯新闻',u:'https://news.qq.com'}];
const GROUP_LINKS=[{n:'📢 腾讯频道·花学社',u:'https://pd.qq.com/s/he8q7kzm3?b=9'},{n:'💬 QQ群·花学交流群',u:'https://qun.qq.com/universal-share/share?ac=1&authKey=1aba71zRbWzorsI0HDNexE225xjziJnCD67y5Uc7ZVPOlQIXv3spYCzX27ZzAKWn&busi_data=eyJncm91cENvZGUiOiI3MzkzMzYwODEiLCJ0b2tlbiI6IjJGK2ZVci9NazE3TEVqZXVTcUhDN2hmQmMyQUNiWXA2MW0wejU2ekc4bytsb0RxZ1VOZDNFMm9qZWV5QjNwamkiLCJ1aW4iOiIyNjI5MTgxNzQ3In0%3D&data=qwk3yj6de7aK__jeez-Czl28fBpOJrnBaPPVnanfnIKpBn_WgrZJ-pTPiT-7KUczKFuimVrahitZ1bkfq4AJXA&svctype=4&tempid=h5_group_info'}];
...
function applyBg(){const b=localStorage.getItem('hxs_bg')||'';document.body.style.background=b?b:'#F0E6FF';}
...
async function refreshToken(){const rt=localStorage.getItem('sb_refresh');if(!rt)throw new Error('no rt');const r=await fetch(SUPABASE_URL_CONFIG+'/auth/v1/token?grant_type=refresh_token',{method:'POST',headers:{'apikey':SUPABASE_ANON_KEY_CONFIG,'Content-Type':'application/json'},body:JSON.stringify({refresh_token:rt})});const j=await r.json();if(j&&j.access_token){localStorage.setItem('sb_token',j.access_token);localStorage.setItem('sb_refresh',j.refresh_token);return j.access_token;}throw new Error('refresh failed');}
async function api(path,options={}){
  let res=await fetch(SUPABASE_URL_CONFIG+path,{...options,headers:{...authHeaders(),...(options.headers||{})}});
  if(res.status===401){try{await refreshToken();res=await fetch(SUPABASE_URL_CONFIG+path,{...options,headers:{...authHeaders(),...(options.headers||{})}});}catch(e){logout();throw new Error('登录已过期，请重新登录');}}
  if(!res.ok){let msg='HTTP '+res.status;try{const j=await res.json();msg=j.msg||j.error_message||j.message||j.hint||msg;}catch(e){}throw new Error(msg);}
  const txt=await res.text();if(!txt)return null;try{return JSON.parse(txt);}catch(e){return txt;}}
...
let currentBoard='jinghua',currentPost=null,myProfile=null,currentSort='new',currentTipAmount=1,currentRank='good',currentResType='video',curFavType='app',searchMode=false,isReg=false,captchaCode='',adIdx=0,adTimer=null,cmImageData=null,currentUserPage=null,replyingTo=null,afkTimer=null,afkCode='',spotlightTimer=null,pomoTimer=null,pomoLeft=1500;

applyBg();
function needLogin(){goPage('loginPage');}
function goMe(){if(myProfile)goPage('mePage');else needLogin();}
function goPost(){if(myProfile)goPage('postPage');else{toast('请先登录');needLogin();}}
function goInfo(){if(myProfile)goPage('infoPage');else needLogin();}
function goGuest(){myProfile=null;localStorage.removeItem('sb_token');localStorage.removeItem('sb_refresh');localStorage.removeItem('sb_user');goPage('mainPage');initAds();loadSpotlight();}
function goPage(id){
  ...
  if(id==='postPage'){if(myProfile)initPostForm();else needLogin();}
  if(id==='mePage'){if(myProfile){loadMe();loadHistory();}else needLogin();}
  if(id==='editPage'){if(myProfile)initEdit();else needLogin();}
  if(id==='invitePage'){if(myProfile)loadInvite();else needLogin();}
  if(id==='groupPage')loadGroup();
  if(id==='signPage'){if(myProfile)loadSignPage();else needLogin();}
  ...
  if(id==='resPostPage'){if(myProfile&&myProfile.role==='admin')initResPost();else needLogin();}
  if(id==='adminPage'){if(myProfile&&myProfile.role==='admin')loadAdmin('reports');else needLogin();}
  if(id==='coursePage')renderCourses();
  if(id==='infoPage'){if(myProfile)loadInfo();else needLogin();}
  if(id==='favPage'){if(myProfile)loadFavs(curFavType);else needLogin();}
  if(id==='pomoPage'){if(myProfile)pomoRender();else needLogin();}
...
function drawCaptcha(){captchaCode=randCode();const c=$('captcha'),ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);ctx.fillStyle='#eef2f7';ctx.fillRect(0,0,c.width,c.height);for(let i=0;i<3;i++){ctx.strokeStyle='#c9d4e0';ctx.beginPath();ctx.moveTo(Math.random()*c.width,Math.random()*c.height);ctx.lineTo(Math.random()*c.width,Math.random()*c.height);ctx.stroke();}ctx.font='bold 22px Arial';for(let i=0;i<captchaCode.length;i++){ctx.fillStyle=['#7B68EE','#e5484d','#ff9500','#6A5ACD'][i%4];ctx.save();ctx.translate(18+i*20,26);ctx.rotate((Math.random()-0.5)*0.4);ctx.fillText(captchaCode[i],0,0);ctx.restore();}}
...
async function doLogin(){
  ...
  try{
    if(isReg){
      ...
      if(r&&r.access_token){localStorage.setItem('sb_token',r.access_token);localStorage.setItem('sb_refresh',r.refresh_token);localStorage.setItem('sb_user',JSON.stringify(r.user));await afterLogin(r.user);if(invite)dbRpc('apply_invite',{p_invite_code:invite.toUpperCase()}).then(x=>{if(x==='OK')toast('🎉邀请成功，对方+25硬币');}).catch(()=>{});}
      ...
      if(r&&r.access_token){localStorage.setItem('sb_token',r.access_token);localStorage.setItem('sb_refresh',r.refresh_token);localStorage.setItem('sb_user',JSON.stringify(r.user));afterLogin(r.user);}
      else{err.textContent='登录失败：QQ号或密码错误，未注册请先注册';}
    }
  }catch(e){err.textContent='登录失败：'+e.message+(e.message.includes('Invalid')?'，未注册请先注册':'');}
}
...
async function logout(){localStorage.removeItem('sb_token');localStorage.removeItem('sb_refresh');localStorage.removeItem('sb_user');location.reload();}
...
function drawAfkCaptcha(){afkCode=randCode();const c=$('afkCaptcha'),ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);ctx.fillStyle='#eef2f7';ctx.fillRect(0,0,c.width,c.height);for(let i=0;i<4;i++){ctx.strokeStyle='#c9d4e0';ctx.beginPath();ctx.moveTo(Math.random()*c.width,Math.random()*c.height);ctx.lineTo(Math.random()*c.width,Math.random()*c.height);ctx.stroke();}ctx.font='bold 24px Arial';for(let i=0;i<afkCode.length;i++){ctx.fillStyle=['#7B68EE','#e5484d','#ff9500','#6A5ACD'][i%4];ctx.save();ctx.translate(24+i*24,28);ctx.rotate((Math.random()-0.5)*0.4);ctx.fillText(afkCode[i],0,0);ctx.restore();}}
...
async function loadSpotlight(){
  if(currentBoard!=='jinghua'){$('spotlight').style.display='none';return;}
  $('spotlight').style.display='block';
  try{
    let h='<div class="dc-grad">🌸 每日速递</div>';
    try{const n=await fetch('https://api.magicalbox.cn/api3/tool/api/getMorningPaper');const nj=await n.json();if(nj&&nj.data){h+='<div class="daily-card"><div class="dc-title">» 60秒早报 <span style="margin-left:auto;color:#999;font-size:10px">'+nj.data.date+'</span></div>';if(nj.data.news)nj.data.news.slice(0,12).forEach((x,i)=>{h+='<div class="dc-item" onclick="quickPost(\''+esc(x.slice(0,20))+'…\')"><b>'+(i+1)+'.</b> '+esc(x)+'</div>';});if(nj.data.image)h+='<div class="dc-item" style="color:#7B68EE" onclick="location.href=\''+nj.data.image+'\'">🖼️ 查看完整早报图 →</div>';h+='</div>';if(myProfile&&!localStorage.getItem('hxs_sp_'+nj.data.date)){localStorage.setItem('hxs_sp_'+nj.data.date,'1');try{await dbRpc('notify_user',{p_user_id:myProfile.id,p_type:'coin',p_content:'阅读今日简报+1经验+1硬币'});await dbUpdate('profiles','id=eq.'+myProfile.id,{exp:myProfile.exp+1,coins:myProfile.coins+1});myProfile.exp+=1;myProfile.coins+=1;}catch(e){}}}}catch(e){}
    try{const e=await fetch('https://apiv3.shanbay.com/weapps/dailyquote/quote');const ej=await e.json();if(ej&&ej.content)h+='<div class="daily-card"><div class="dc-title">» 每日英语</div><div class="dc-en">"'+esc(ej.content)+'"</div>'+(ej.translation?'<div class="dc-cn">'+esc(ej.translation)+(ej.author?' · '+esc(ej.author):'')+'</div>':'')+'</div>';}catch(e){}
    try{const q=await fetch('https://v1.hitokoto.cn/');const qj=await q.json();if(qj&&qj.hitokoto)h+='<div class="daily-card"><div class="dc-title">» 每日一言</div><div class="dc-quote">"'+esc(qj.hitokoto)+'"</div>'+(qj.from?'<div class="dc-cn" style="text-align:center">—— '+esc(qj.from)+'</div>':'')+'</div>';}catch(e){}
    try{const g=await fetch('http://8.134.118.1:9090/days/apps/getHistory');const gt=await g.text();const parts=gt.split('%%%').filter(x=>x.trim()&&x.trim()!=='true');let hidx=0;h+='<div class="daily-card"><div class="dc-title">» 历史上的今天</div>';parts.forEach(p=>{try{const j=JSON.parse(p.trim());if(j&&j.title&&hidx<3){h+='<div class="dc-item"><b>'+esc(String(j.year||'').replace('年',''))+'</b>· '+esc(String(j.title).replace(/<[^>]+>/g,''))+'</div>';hidx++;}}catch(e){}});h+='</div>';}catch(e){}
    h+='<div class="daily-card"><div class="dc-title">» 实时热搜</div><div>';
    HOT_LINKS.forEach(x=>{h+='<span class="dc-tag" onclick="location.href=\''+x.u+'\'">'+x.n+'</span>';});
    h+='</div></div>';
    $('spotlight').innerHTML=h;
  }catch(e){$('spotlight').innerHTML='<div class="daily-card"><div class="empty">日报加载失败</div></div>';}
  if(spotlightTimer)clearInterval(spotlightTimer);
  spotlightTimer=setInterval(()=>{if(currentBoard==='jinghua')loadSpotlight();},28800000);
}
function quickPost(title){if(!myProfile){toast('请先登录');needLogin();return;}goPage('postPage');$('pTitle').value=title.slice(0,40);$('pContent').focus();}
...
function doSearch(){searchMode=true;const kw=$('searchInput').value.trim();const box=$('postList');box.innerHTML='<div class="empty">搜索中...</div>';api('/rest/v1/posts?select=*,author_id(nickname,level,qq,id)&status=eq.published&or=(title.ilike.*'+enc(kw)+'*,content.ilike.*'+enc(kw)+'*)&order=created_at.desc&limit=50').then(data=>renderPostCards(data,box,'没搜到相关帖子')).catch(e=>box.innerHTML='<div class="empty">搜索失败</div>');}
...
async function openPost(id){
  ...
  const data=currentPost,fav=myProfile?await isFaved('post',data.id):false;const isAuthor=myProfile&&data.author_id&&data.author_id.id===myProfile.id;
  ...
  $('cmBar').style.display=myProfile?'flex':'none';replyingTo=null;loadComments(id);goPage('detailPage');
}
function isLiked(id){return(JSON.parse(localStorage.getItem('liked')||'[]')).includes(id);}
async function likePost(){if(!myProfile){toast('请先登录');needLogin();return;}const l=JSON.parse(localStorage.getItem('liked')||'[]');if(l.includes(currentPost.id))return;l.push(currentPost.id);localStorage.setItem('liked',JSON.stringify(l));try{await dbUpdate('posts','id=eq.'+currentPost.id,{like_count:currentPost.like_count+1});currentPost.like_count++;if(currentPost.author_id&&currentPost.author_id.id!==myProfile.id)dbRpc('notify_user',{p_user_id:currentPost.author_id.id,p_type:'like',p_content:myProfile.nickname+'赞了你的帖子'}).catch(()=>{});document.querySelectorAll('.action-btn')[0].classList.add('on');}catch(e){toast('点赞失败');}}
...
async function deletePost(pid){if(!confirm('确定删除此帖？'))return;try{const r=await dbRpc('delete_post',{pid});toast(r==='OK'?'✅已删除':('⚠️'+r));if(r==='OK')goPage('mainPage');}catch(e){toast('删除失败：'+e.message);}}
...
function pickImg(inp,prevId){const f=inp.files&&inp.files[0];if(!f)return;const prev=$(prevId);const img=URL.createObjectURL(f);prev.innerHTML=`<div class="img-preview-wrap"><img src="${img}"><span class="cancel-img" onclick="cancelPickImg('${prevId}','${inp.id}')">✕</span></div>`;inp._selected=f;}
...
async function likeComment(cid){if(!myProfile){toast('请先登录');needLogin();return;}try{const cur=await api('/rest/v1/comments?select=like_count&id=eq.'+cid);await dbUpdate('comments','id=eq.'+cid,{like_count:(cur&&cur[0]?cur[0].like_count:0)+1});toast('👍已点赞');loadComments(currentPost.id);}catch(e){toast('点赞失败');}}
function replyTo(cid,name){if(!myProfile){toast('请先登录');needLogin();return;}replyingTo=cid;$('cmInput').placeholder='回复 @'+name+'…';$('cmInput').focus();$('cmBar').style.display='flex';}
async function submitComment(){if(!myProfile){toast('请先登录');needLogin();return;}const content=$('cmInput').value.trim();if(!content&&!cmImageData)return;
...
async function uploadImage(file){const name=Date.now()+'-'+Math.random().toString(36).slice(2,6)+'.jpg';const res=await fetch(SUPABASE_URL_CONFIG+'/storage/v1/object/images/'+name,{method:'POST',headers:{...authHeaders(),'Content-Type':file.type||'image/jpeg'},body:file});if(!res.ok){let info='';try{info=await res.text();}catch(e){}if(res.status===404)throw new Error('Storage没有images桶，请到Supabase→Storage创建Public桶images');throw new Error('上传失败('+res.status+')'+info.slice(0,100));}return SUPABASE_URL_CONFIG+'/storage/v1/object/public/images/'+name;}
...
async function submitPost(){
  if(!myProfile){toast('请先登录');needLogin();return;}
...
function openTip(){if(!myProfile){toast('请先登录');needLogin();return;}if(currentPost.author_id&&currentPost.author_id.id===myProfile.id){toast('不能打赏自己');return;}$('tipCoins').innerHTML=[1,2,5,10].map(n=>`<div class="chip ${n===currentTipAmount?'on':''}" data-n="${n}" onclick="pickTip(${n})">🪙${n}</div>`).join('');$('tipModal').classList.add('show');}
...
let editBtn=isMe?'<div class="uedit" onclick="goPage(\'editPage\')">✏️ 编辑资料</div>':'';
let folBtn=isMe?'':`<span class="follow-btn ${fol&&fol.length?'':'on'}" style="background:${fol&&fol.length?'#eee':'linear-gradient(135deg,#7B68EE,#6A5ACD)'};color:${fol&&fol.length?'#555':'#fff'}" onclick="${fol&&fol.length?'unfollowUser(\''+uid+'\')':'followUser(\''+uid+'\')'}">${fol&&fol.length?'已关注':'＋关注'}</span>`;
box.innerHTML=`<div class="ubg">${u.bg_image?`<img src="${esc(u.bg_image)}" onerror="this.parentNode.style.background='linear-gradient(135deg,#7B68EE,#6A5ACD)'">`:'<div style="width:100%;height:100%"></div>'}<div class="ubg-avatar">${u.avatar_url?`<img src="${esc(u.avatar_url)}">`:esc((u.nickname||'友')[0])}</div></div><div class="uinfo"><div class="uname">${esc(u.nickname)}${u.role==='admin'?'<span class="utag">管理员</span>':''}</div>${(myProfile&&myProfile.role==='admin'&&!isMe)?`<div class="uq">QQ：${esc(u.qq||'-')}</div>`:''}<div class="pt-exp"><i style="width:${Math.max(4,pct)}%"></i></div><div class="pt-exp-t">Lv${u.level}·经验${u.exp}/${expNeedLv(u.level+1)}</div><div class="profile-bio">${esc(u.bio||'这个人很懒，什么都没写')}</div><div class="stat-card" style="margin-top:8px"><div><div class="stat-num" style="color:#ff9500">🪙${u.coins}</div><div class="stat-label">硬币</div></div><div><div class="stat-num" style="color:#7B68EE">${folN?folN.length:0}</div><div class="stat-label">关注</div></div><div><div class="stat-num" style="color:#6A5ACD">${fanN?fanN.length:0}</div><div class="stat-label">粉丝</div></div></div>${editBtn}${folBtn}</div>`;loadUserPosts('posts');goPage('userPage');}catch(e){toast('加载失败');}}
...
async function followUser(uid){if(!myProfile){toast('请先登录');needLogin();return;}try{const r=await dbRpc('follow_user',{p_following_id:uid});toast(r==='OK'?'✅已关注':r);openUserPage(uid);}catch(e){toast('操作失败');}}
...
const GRID=[['📅','每日签到','signPage'],['⭐','我的收藏','favPage'],['🏆','排行榜','rankPage'],['🎟️','邀请好友','invitePage'],['💬','交流群','groupPage'],['🛡️','管理员后台','adminPage'],['🍅','番茄时钟','pomoPage'],['🎨','背景美化','editPage'],['🚪','退出登录','logout']];
...
$('meProfile').innerHTML=`<div class="me-card" onclick="openUserPage('${u.id}')"><div class="me-avatar">${u.avatar_url?`<img src="${esc(u.avatar_url)}">`:esc((u.nickname||'友')[0])}</div><div class="me-info"><div class="me-name">${esc(u.nickname)}${u.role==='admin'?'<span class="pt-tag" style="background:#e5484d;font-size:9px;color:#fff;border-radius:3px;padding:1px 4px;margin-left:4px">管理员</span>':''}<span style="font-size:10px;color:#6A5ACD;margin-left:6px">💯${u.credit}分</span></div><div class="me-exp"><i style="width:${Math.max(4,pct)}%"></i></div><div class="me-exp-t">Lv${u.level}·经验${u.exp}/${expNeedLv(u.level+1)}·${checkinLevelTxt(u)}</div></div><div class="me-arrow">›</div></div>`;
...
function loadGroup(){$('groupBox').innerHTML='<div class="daily-card"><div class="dc-title">💬 加入花学社交流群</div>'+GROUP_LINKS.map(g=>`<div class="his-card" onclick="location.href='${g.u}'"><span class="ht" style="flex:1;font-size:14px">${g.n}</span><span style="color:#7B68EE">›</span></div>`).join('')+'</div><div class="tip" style="text-align:center">点击卡片加入，学习交流不迷路</div>';}
...
function initEdit(){const chips=$('bgChips');const bgs=[['默认',''],['浅蓝','linear-gradient(180deg,#e8f1ff,#F0E6FF)'],['浅绿','linear-gradient(180deg,#e6f7ec,#F0E6FF)'],['浅紫','linear-gradient(180deg,#efe9ff,#F0E6FF)'],['暖黄','linear-gradient(180deg,#fff7e0,#F0E6FF)']];const cur=localStorage.getItem('hxs_bg')||'';chips.innerHTML=bgs.map((b,i)=>`<div class="chip ${cur===b[1]?'on':''}" data-v="${i}" onclick="pickBg(this,${i})">${b[0]}</div>`).join('');$('eBio').value=myProfile.bio||'';$('editErr').textContent='';$('avPreview').innerHTML='';$('bgPreview').innerHTML='';}
function pickBg(el,i){const bgs=[[''],['linear-gradient(180deg,#e8f1ff,#F0E6FF)'],['linear-gradient(180deg,#e6f7ec,#F0E6FF)'],['linear-gradient(180deg,#efe9ff,#F0E6FF)'],['linear-gradient(180deg,#fff7e0,#F0E6FF)']];localStorage.setItem('hxs_bg',bgs[i]);document.querySelectorAll('#bgChips .chip').forEach(x=>x.classList.remove('on'));el.classList.add('on');applyBg();toast('🎨已应用');}
async function saveProfile(){if(!myProfile){toast('请先登录');needLogin();return;}const bio=$('eBio').value.trim();try{let avatar_url=myProfile.avatar_url||'';let bg_image=myProfile.bg_image||'';const fa=$('eAvatar')._selected;if(fa){avatar_url=await uploadImage(fa);}const fb=$('eBg')._selected;if(fb){bg_image=await uploadImage(fb);}await dbUpdate('profiles','id=eq.'+myProfile.id,{bio,avatar_url,bg_image});myProfile.bio=bio;myProfile.avatar_url=avatar_url;myProfile.bg_image=bg_image;toast('✅已保存');loadMe();goPage('mePage');}catch(e){$('editErr').textContent='保存失败：'+e.message;}}

async function loadInvite(){if(!myProfile)return;$('inviteBox').innerHTML=`<div class="profile-top"><div style="font-size:14px;color:#999">我的邀请码</div><div style="font-size:30px;font-weight:800;color:#7B68EE;margin:6px 0;letter-spacing:3px">${esc(myProfile.invite_code||'---')}</div><button class="btn" style="max-width:200px" onclick="copyInvite()">📋复制邀请码</button><div class="tip" style="text-align:center">好友注册时填写你的邀请码<br>每成功邀请1人+25硬币（月限5次）</div></div>`;}
...
async function loadRank(type){currentRank=type;$('tabGood').classList.toggle('on',type==='good');if($('tabLike'))$('tabLike').classList.toggle('on',type==='like');$('tabLevel').classList.toggle('on',type==='level');const box=$('rankList');box.innerHTML='<div class="empty">加载中...</div>';try{if(type==='good'){const data=await api('/rest/v1/posts?select=*,author_id(nickname,level)&status=eq.published&order=good_count.desc&limit=20');box.innerHTML=(!data||!data.length)?'<div class="empty">暂无数据</div>':data.map((p,i)=>`<div class="rank-item"><div class="rank-no ${i<3?'top'+(i+1):''}">${i+1}</div><div style="flex:1;margin-left:6px"><div style="font-size:13px">${esc(p.title)}</div><div style="font-size:11px;color:#999">👤${esc(p.author_id?.nickname||'')}·${fmtTime(p.created_at)}</div></div><div class="rank-val">❤️${p.good_count}</div></div>`).join('');}else if(type==='like'){const data=await api('/rest/v1/posts?select=*,author_id(nickname,level)&status=eq.published&order=like_count.desc&limit=20');box.innerHTML=(!data||!data.length)?'<div class="empty">暂无数据</div>':data.map((p,i)=>`<div class="rank-item"><div class="rank-no ${i<3?'top'+(i+1):''}">${i+1}</div><div style="flex:1;margin-left:6px"><div style="font-size:13px">${esc(p.title)}</div><div style="font-size:11px;color:#999">👤${esc(p.author_id?.nickname||'')}</div></div><div class="rank-val">👍${p.like_count}</div></div>`).join('');}else{const data=await api('/rest/v1/profiles?select=*&order=exp.desc&limit=20');box.innerHTML=(!data||!data.length)?'<div class="empty">暂无数据</div>':data.map((u,i)=>`<div class="rank-item"><div class="rank-no ${i<3?'top'+(i+1):''}">${i+1}</div><div style="flex:1;margin-left:6px;font-size:13px">${esc(u.nickname)}${u.role==='admin'?'<span class="lv" style="background:#e5484d">管理</span>':''}</div><div class="rank-val"><span class="lv" style="background:#7B68EE">Lv${u.level}</span>⚡${u.exp}</div></div>`).join('');}}catch(e){box.innerHTML='<div class="empty">加载失败</div>';}}

function pomoStart(){const b=$('pomoBtn');if(pomoTimer){clearInterval(pomoTimer);pomoTimer=null;b.textContent='继续';$('pomoState').textContent='已暂停';return;}const h=parseInt($('pomoH').value)||0;const m=parseInt($('pomoM').value)||0;const s=parseInt($('pomoS').value)||0;pomoLeft=h*3600+m*60+s;if(pomoLeft<=0){toast('请设置时间');return;}b.textContent='暂停';$('pomoState').textContent='专注中…';pomoTimer=setInterval(()=>{pomoLeft--;if(pomoLeft<=0){clearInterval(pomoTimer);pomoTimer=null;b.textContent='开始';$('pomoState').textContent='🍅专注完成！';toast('🍅番茄完成！');return;}pomoRender();},1000);}
function pomoReset(){if(pomoTimer){clearInterval(pomoTimer);pomoTimer=null;}$('pomoBtn').textContent='开始';$('pomoState').textContent='自定义时/分/秒';pomoLeft=1500;pomoRender();}
function pomoRender(){const h=String(Math.floor(pomoLeft/3600)).padStart(2,'0'),m=String(Math.floor(pomoLeft%3600/60)).padStart(2,'0'),s=String(pomoLeft%60).padStart(2,'0');$('pomoTime').textContent=(h>0?h+':':'')+m+':'+s;}
function renderCourses(){$('courseList').innerHTML=COURSES.map((c,i)=>`<div class="his-card" onclick="location.href='${c.u}'"><div class="ht" style="flex:1;font-size:13px">${c.p==='bili'?'🔵':'🔴'}${c.t}</div><span style="font-size:11px;color:${c.p==='bili'?'#fb7299':'#161823'}">${c.p==='bili'?'B站':'抖音'}</span><span style="color:#6A5ACD;font-size:12px;cursor:pointer;margin-left:6px" onclick="event.stopPropagation();favCourse(${i})">⭐</span></div>`).join('');}
async function favCourse(idx){if(!myProfile){toast('请先登录');needLogin();return;}try{const r=await api('/rest/v1/favorites?user_id=eq.'+myProfile.id+'&target_type=eq.course&target_id=eq.'+idx+'&limit=1');if(r&&r.length){await dbDelete('favorites','user_id=eq.'+myProfile.id+'&target_type=eq.course&target_id=eq.'+idx);toast('已取消收藏');}else{await dbInsert('favorites',{user_id:myProfile.id,target_type:'course',target_id:idx});toast('⭐已收藏');}}catch(e){toast('操作失败');}}
...
async function loadResources(type){currentResType=type;$('resTab1').classList.toggle('on',type==='video');$('resTab2').classList.toggle('on',type==='app');const box=$('resList');box.innerHTML='<div class="empty">加载中...</div>';try{const data=await api('/rest/v1/resources?select=*&type=eq.'+type+'&order=created_at.desc');if(!data||!data.length){box.innerHTML='<div class="empty">暂无内容</div>';return;}const isAdmin=myProfile&&myProfile.role==='admin';if(type==='video'){box.innerHTML=data.map(r=>`<div class="video-card"><div class="video-cover" onclick="location.href='${esc(r.link)}'">▶️</div><div class="video-info"><div class="vt">${esc(r.title)}<span class="lv" style="margin-left:4px;background:#7B68EE">${r.pan_type||'外站'}</span></div><div class="vs">${esc(r.description||'')}·点击观看</div>${isAdmin?`<div style="margin-top:4px"><span style="color:#e5484d;font-size:11px;cursor:pointer" onclick="delResource(${r.id})">🗑删除</span></div>`:''}</div></div>`).join('');}else{box.innerHTML=data.map(r=>`<div class="app-card" onclick="openAppDetail(${r.id})"><div class="ac-top"><div class="ac-icon" style="background:#eee;text-align:center;line-height:48px;font-size:22px">${r.icon?`<img src="${esc(r.icon)}" class="ac-icon">`:'📱'}</div><div><div class="ac-title">${esc(r.title)}<span class="lv" style="margin-left:4px;background:#ff9500">${r.size_mb||''}</span></div><div class="ac-sub">${esc(r.description||'')}·📥${r.downloads||0}次下载</div>${isAdmin?`<div style="margin-top:4px"><span style="color:#e5484d;font-size:11px;cursor:pointer" onclick="event.stopPropagation();delResource(${r.id})">🗑删除</span></div>`:''}</div></div></div>`).join('');}}catch(e){box.innerHTML='<div class="empty">加载失败</div>';}}
async function delResource(rid){if(!confirm('确定删除该资源？'))return;try{await dbDelete('resources','id=eq.'+rid);toast('已删除');loadResources(currentResType);}catch(e){toast('删除失败');}}
function initResPost(){$('resTypeChips').innerHTML=[['video','🎬教程视频'],['app','📱应用']].map(x=>`<div class="chip ${x[0]===currentResType?'on':''}" data-k="${x[0]}" onclick="pickResType(this,'${x[0]}')">${x[1]}</div>`).join('');$('resErr').textContent='';}
...
async function openAppDetail(id){try{const r=await api('/rest/v1/resources?id=eq.'+id);const d=r&&r[0];if(!d)return;const fav=myProfile?await isFaved('app',d.id):false;$('appDetailBox').innerHTML=`<div style="background:#fff;border-radius:12px;padding:16px;text-align:center;margin-bottom:10px">${d.icon?`<img src="${esc(d.icon)}" style="width:72px;height:72px;border-radius:14px;object-fit:cover;margin-bottom:8px">`:`<div style="width:72px;height:72px;border-radius:14px;background:#eee;line-height:72px;margin:0 auto 8px;font-size:28px">📱</div>`}<div style="font-size:17px;font-weight:700">${esc(d.title)}</div><div class="pt-tag" style="margin-top:4px">📱应用</div></div><div style="background:#fff;border-radius:12px;padding:14px;font-size:13px;line-height:1.7;color:#444;margin-bottom:10px">${esc(d.description||'暂无介绍')}</div><div class="stat-card"><div><div class="stat-num">${d.downloads||0}</div><div class="stat-label">下载次数</div></div><div><div class="stat-num">${d.size_mb||'--'}</div><div class="stat-label">大小</div></div></div><div class="detail-actions"><button class="action-btn fav ${fav?'on':''}" onclick="favResource('app',${d.id})">⭐${fav?'已收藏':'收藏'}</button><button class="action-btn warn" onclick="location.href='${esc(d.link)}'">🔗前往下载</button>${(myProfile&&myProfile.role==='admin')?`<button class="action-btn" style="color:#e5484d" onclick="delResource(${d.id})">🗑删除</button>`:''}</div>`;goPage('appDetailPage');}catch(e){toast('加载失败');}}
async function favResource(type,rid){if(!myProfile){toast('请先登录');needLogin();return;}try{const r=await api('/rest/v1/favorites?user_id=eq.'+myProfile.id+'&target_type=eq.'+type+'&target_id=eq.'+rid+'&limit=1');if(r&&r.length){await dbDelete('favorites','user_id=eq.'+myProfile.id+'&target_type=eq.'+type+'&target_id=eq.'+rid);toast('已取消收藏');}else{await dbInsert('favorites',{user_id:myProfile.id,target_type:type,target_id:rid});toast('⭐已收藏');}loadResources(currentResType);}catch(e){toast('操作失败');}}

async function loadAdmin(tab){$('aTab1').classList.toggle('on',tab==='reports');if($('aTab2'))$('aTab2').classList.toggle('on',tab==='punish');if($('aTab3'))$('aTab3').classList.toggle('on',tab==='posts');const box=$('adminBox');if(tab==='posts'){try{const data=await api('/rest/v1/posts?select=*,author_id(nickname,level)&status=eq.published&order=created_at.desc&limit=100');box.innerHTML='<div class="tip" style="margin-bottom:8px">共'+data.length+'个帖子 · 勾选后批量删除</div><div class="rc-btns" style="margin-bottom:8px"><button class="btn" onclick="selectAllPosts()">全选</button><button class="btn" style="background:#e5484d" onclick="delSelectedPosts()">删除选中</button></div><div id="adminPostList">'+data.map(p=>`<label style="display:flex;align-items:center;padding:8px 0;border-bottom:1px solid #f4f4f4;font-size:13px;cursor:pointer"><input type="checkbox" class="ap" value="${p.id}" style="margin-right:8px">${esc(p.title)}<span style="margin-left:auto;color:#999;font-size:11px">👤${esc(p.author_id?.nickname||'')}·👍${p.like_count}</span></label>`).join('')+'</div>';}catch(e){box.innerHTML='<div class="empty">加载失败</div>';}}else if(tab==='reports'){try{const data=await api('/rest/v1/reports?status=eq.pending&order=created_at.desc');if(!data||!data.length){box.innerHTML='<div class="empty">没有待处理的举报🎉</div>';return;}let html='<div class="tip" style="margin-bottom:8px">共'+data.length+'条待处理举报</div>';for(const r of data){let target='',content='';try{if(r.target_type==='post'){const p=await api('/rest/v1/posts?id=eq.'+r.target_id);if(p&&p[0]){target='帖子：'+p[0].title;content=p[0].content;}}else{const c=await api('/rest/v1/comments?id=eq.'+r.target_id);if(c&&c[0]){target='评论#'+r.target_id;content=c[0].content;}}}catch(e){}html+=`<div class="report-card" style="background:#fff;border-radius:10px;padding:10px;margin-bottom:8px"><div style="font-size:13px;font-weight:600">🚩举报（${esc(r.reason)}）</div><div style="font-size:11px;color:#999;margin:4px 0">${esc(target)}</div><div class="rc-body">${esc(content)}</div><div class="rc-btns"><button class="btn" style="background:#999" onclick="handleReport(${r.id},'ignore')">忽略</button><button class="btn" onclick="handleReport(${r.id},'delete')">删内容</button><button class="btn" style="background:#e5484d" onclick="handleReport(${r.id},'punish')">删+扣分</button></div></div>`;}box.innerHTML=html;}catch(e){box.innerHTML='<div class="empty">加载失败</div>';}}else{box.innerHTML=`<div class="tip" style="margin-bottom:8px">按QQ号查找用户，然后选择处罚</div><input id="punishQQ" class="inp" placeholder="输入用户QQ号"><button class="btn" onclick="findUser()">查找用户</button><div id="userInfo" style="margin-top:10px"></div>`;}}
function selectAllPosts(){document.querySelectorAll('.ap').forEach(x=>x.checked=true);}
async function delSelectedPosts(){const sel=[...document.querySelectorAll('.ap:checked')].map(x=>x.value);if(!sel.length){toast('请先选择帖子');return;}if(!confirm('确定删除选中的'+sel.length+'个帖子？'))return;for(const id of sel){try{await dbRpc('delete_post',{pid:id});}catch(e){}}toast('✅已删除');loadAdmin('posts');}
