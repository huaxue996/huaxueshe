...
function goPage(id){
  ...
  if(id==='mePage'){if(myProfile){loadMe();loadHistory();}else{toast('请先登录');goPage('mainPage');}}
...
async function showMain(){
  isGuest=false;myProfile=await fetchProfile();if(!myProfile)myProfile={nickname:'花友',level:1,exp:0,coins:0,credit:100,role:'user'};
...
async function loadSpotlight(){
  ...
  try{
    ...
    try{
      ...
      if(nj&&nj.data){
        h+='<div class="daily-card"><div class="dc-title">» 60秒读懂世界 <span style="margin-left:auto;color:#999;font-size:11px">'+nj.data.date+' '+nj.data.day_of_week+' '+nj.data.lunar_date+'</span></div>';
        if(nj.data.news)nj.data.news.forEach((x,i)=>{h+='<div class="dc-item" onclick="quickPost(\''+esc(x.slice(0,20))+'…\')"><b>'+(i+1)+'.</b> '+esc(x)+'</div>';});
...
async function likeComment(cid){if(!myProfile){toast('请先登录');return;}try{const cur=await api('/rest/v1/comments?select=like_count&id=eq.'+cid);await dbUpdate('comments','id=eq.'+cid,{like_count:(cur&&cur[0]?cur[0].like_count:0)+1});toast('👍已点赞');loadComments(currentPost.id);}catch(e){toast('点赞失败');}}
function replyTo(cid,name){if(!myProfile){toast('请先登录');return;}replyingTo=cid;$('cmInput').placeholder='回复 @'+name+'…';$('cmInput').focus();$('cmBar').style.display='flex';}
async function submitComment(){if(!myProfile){toast('请先登录');return;}const content=$('cmInput').value.trim();if(!content&&!cmImageData)return;
...
async function submitPost(){
  if(!myProfile){toast('请先登录');return;}
...
async function followUser(uid){if(!myProfile){toast('请先登录');return;}try{const r=await dbRpc('follow_user',{p_following_id:uid});toast(r==='OK'?'✅已关注':r);openUserPage(uid);}catch(e){toast('操作失败');}}
...
async function saveProfile(){if(!myProfile){toast('请先登录');return;}const bio=$('eBio').value.trim();try{let avatar_url=myProfile.avatar_url||'';const f=$('eAvatar')._selected;if(f){avatar_url=await uploadImage(f);}await dbUpdate('profiles','id=eq.'+myProfile.id,{bio,avatar_url});myProfile.bio=bio;myProfile.avatar_url=avatar_url;toast('✅已保存');loadMe();goPage('mePage');}catch(e){$('editErr').textContent='保存失败：'+e.message;}}
